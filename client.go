...
func (self *Client) handleResp(res *Response) (err error) {
	...
	if res.StatusCode == 301 {
		if err = self.handle301(res); err != nil {
			return
		}
	}
...
func (self *Client) handle301(res *Response) (err error) {
	/*
		RTSP/1.0 200 OK
		CSeq: 301
	*/
...
func (self *Client) Describe() (streams []sdp.Media, err error) {
	...
	// 301处理
	if res.ContentLength == 0 && res.StatusCode == 301{
		fmt.Printf("\n> MOVED rtsp: 301 Moved Permanently, Url=%s\n",self.requestUri)
		// 302处理
	}else if res.ContentLength == 0 && res.StatusCode == 302{
		fmt.Printf("\n> MOVED rtsp: 302 Moved Temporarily, Url=%s\n",self.requestUri)
	}else{
		err = fmt.Errorf("\nrtsp: Describe failed, StatusCode=%d", res.StatusCode)
		return
	}
	...
	if self.DebugRtsp && res.Body != nil {
		fmt.Println("<", body)
	}
